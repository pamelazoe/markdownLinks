...
const addPath = async (dirname, inputPath, options) => {
	const ifFile = isFile(inputPath)
	const ifDir = isDir(inputPath)
	const mdFile = isMdFile(inputPath)
	const array = []
	if (mdFile === true) {
...
// addPath("README2.md", { validate: false, stats: false })
